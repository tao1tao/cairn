...
from cairn.dispatcher.tasks.common import best_effort_release as common_best_effort_release
from cairn.dispatcher.tasks.common import best_effort_release_reason
...
class DispatcherLoop:
    def __init__(self, config_path: Path):
        ...
        self.bootstrap_retries: dict[str, int] = {}  # project_id -> retry_count
    ...
    def _reap_futures(self) -> None:
        ...
        for future in done:
            ...
            try:
                ...
                if outcome == "success" and task.task_type == "bootstrap":
                    self.bootstrap_retries.pop(task.project_id, None)
                if outcome in ("failed", "rejected") and task.task_type == "bootstrap":
                    retries = self.bootstrap_retries.get(task.project_id, 0) + 1
                    self.bootstrap_retries[task.project_id] = retries
                    max_r = self.config.tasks.bootstrap.max_retries
                    if retries >= max_r:
                        LOG.warning(
                            "bootstrap max retries reached project=%s retries=%s/%s stopping project",
                            task.project_id, retries, max_r,
                        )
                        self.client.set_project_status(task.project_id, "stopped")
                        self.bootstrap_retries.pop(task.project_id, None)
    ...
    def _best_effort_release(self, project_id: str, intent_id: str, worker_name: str) -> None:
        common_best_effort_release(self.client, project_id, intent_id, worker_name)

    def _best_effort_release_reason(self, project_id: str, worker_name: str) -> None:
        best_effort_release_reason(self.client, project_id, worker_name)
    ...
    def _prune_log_state(self) -> None:
        """Remove entries for projects that are no longer tracked, keeping only global scopes."""
        known = {"dispatch/global", "dispatch/idle-limit"}
        for scope in list(self._log_state):
            if scope in known:
                continue
            if scope.startswith("project:"):
                proj_id = scope.split(":")[1]
                if proj_id not in self.runtime_project_ids:
                    self._log_state.pop(scope, None)
