...
def run_reason_task(
    ...
    try:
        ...
        LOG.debug(
            ...
            len(project.facts), len(allowed_fact_ids),
            len(project.hints), len(open_intents),
        )
        ...
        if did_timeout(result):
            LOG.warning(
                "reason timed out project=%s worker=%s "
        ...
        if result.returncode != 0:
            LOG.warning(
                "reason command failed project=%s worker=%s code=%s "
                "execute_ms=%s total_ms=%s stdout_preview=%s stderr_preview=%s",
                project.project.id, worker.name, result.returncode,
                execute_ms, total_ms,
                preview(result.stdout), preview(result.stderr),
            )
            return "failed"
        ...
        try:
            ...
            LOG.warning(
                "reason parse failed project=%s worker=%s error=%s "
                "execute_ms=%s total_ms=%s stdout_preview=%s stderr_preview=%s",
                project.project.id, worker.name, exc,
                execute_ms, total_ms,
                preview(result.stdout), preview(result.stderr),
            )
            return "failed"
        ...
        if kind == "rejected":
            LOG.warning(
                "reason rejected project=%s worker=%s "
        ...
        if kind == "intents":
            ...
            for intent_data in data:
                response = client.create_intent(
                    project.project.id, intent_data["from"],
                    intent_data["description"], worker.name,
                )
                if response.status_code == 403:
                    LOG.info(
                        "project became inactive during reason intent create "
                        "project=%s worker=%s created=%s",
                        project.project.id, worker.name, created,
                    )
                    return "success"
                if response.status_code == 409:
                    LOG.info(
                        "reason intent lost race project=%s worker=%s from=%s",
                        project.project.id, worker.name, intent_data["from"],
                    )
                    continue
                if not response.ok:
                    LOG.warning(
                ...
                LOG.info(
                    ...
                    project.project.id, worker.name,
                    intent_data["from"], intent_data["description"],
                )
            LOG.info(
                "reason finished project=%s worker=%s created_intents=%s/%s "
                "execute_ms=%s total_ms=%s",
                project.project.id, worker.name, created, len(data),
                execute_ms, total_ms,
            )
            if created == 0:
                LOG.warning(
                    "reason created no intents project=%s worker=%s attempted=%s "
                    "execute_ms=%s total_ms=%s",
                    project.project.id, worker.name, len(data),
                    execute_ms, total_ms,
                )
                return "failed"
            return "success"
        ...
        LOG.info(
            "reason finished without graph change project=%s worker=%s "
            "execute_ms=%s total_ms=%s",
